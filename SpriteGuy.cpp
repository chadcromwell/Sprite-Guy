...
Description: A game made with Allegro, and C++
Copyright info:
...
void dropEggs(BITMAP *dest, SPRITE *spr) {
	...
	draw_compiled_sprite(buffer, eggImg[0], spr->x, spr->y); //Draw the compiled sprite of the egg
...
void dropPotions(BITMAP *dest, SPRITE *spr) {
	...
	draw_compiled_sprite(buffer, potionImg[0], spr->x, spr->y); //Draw potion
...
void dropHearts(BITMAP *dest, SPRITE *spr) {
	...
	draw_compiled_sprite(buffer, heartImg[0], spr->x, spr->y); //Draw the heart
...
void health(BITMAP *dest) {
	...
	if (hp == 3) { //If player has 3 HP show 3 hearts
		masked_blit(hpHeart, buffer, 0, 0, SCREEN_W - 128, 5, 32, 32);
		masked_blit(hpHeart, buffer, 0, 0, SCREEN_W - 86, 5, 32, 32);
		masked_blit(hpHeart, buffer, 0, 0, SCREEN_W - 44, 5, 32, 32);
	}
...
int main(void)
{
	...
	//Background song
	volume = 155;
	bg = load_sample("Sounds/bg.wav"); //Load the bg.wav file, assign to intro
	...
	hurt = load_sample("Sounds/hurt.wav");
	if (!hurt) {
		allegro_message("Couldn't find the hurt.wav file");
		return 1;
	}
	//Healed sound
	healed = load_sample("Sounds/healed.wav");
	if (!healed) {
	...
	//Grow sound
	grow = load_sample("Sounds/grow.wav");
	if (!grow) {
	...
	//Shrink sound
	shrink = load_sample("Sounds/shrink.wav");
	if (!shrink) {
	...
	//Invincible sound
	invincible = load_sample("Sounds/invincible.wav");
	if (!invincible) {
	...
	//***Load Fonts***
	bodyFont = load_font("Fonts/body.pcx", NULL, NULL); //Font for body text
	titleFont = load_font("Fonts/title.pcx", NULL, NULL); //Font for title
	...
	while (true) {
		...
		textprintf_ex(buffer, bodyFont, SCREEN_W / 2 - text_length(bodyFont, "Copyright (c) 2017 Chad Cromwell") / 2, SCREEN_H - 20, BLACK, -1, "Copyright (c) 2017 Chad Cromwell");
		release_bitmap(buffer);
	...
	temp = load_bitmap("Graphics/sand.bmp", NULL); //Load sand.bmp to temp bitmap
	for (int x = 0; x < 16; x++) {
	...
	temp = load_bitmap("Graphics/potion.bmp", NULL);
	potionImg[0] = compiled_grabframe(temp, 32, 32, 0, 0, 1, 0);
	destroy_bitmap(temp);

	temp = load_bitmap("Graphics/egg.bmp", NULL);
	//Compiled Grabforms for the eggs
	eggImg[0] = compiled_grabframe(temp, 32, 32, 0, 0, 1, 0);
	...
	temp = load_bitmap("Graphics/heart.bmp", NULL);
	...
	temp = load_bitmap("Graphics/eggsplat.bmp", NULL);
	blit(temp, splat, 0, 0, 0, 0, 32, 32);
	destroy_bitmap(temp);

	background = load_bitmap("Graphics/bg.bmp", NULL);


	//Load compiled sprite of Sprite Guy
	temp = load_bitmap("Graphics/SG.bmp", NULL);
	...
	while (gameOver == false) {
		...
		while (counter > 0) {
			...
			acquire_bitmap(buffer);
			drawGround(buffer, ground); //Draw the ground
			animate(buffer, spriteGuy); //Animate spriteGuy
			dropEggs(buffer, egg); //Drop eggs
			dropEggs(buffer, egg2);
			dropEggs(buffer, egg3);
			dropEggs(buffer, egg4);
			dropEggs(buffer, egg5);
			dropPotions(buffer, potion); //Drop potions
			dropHearts(buffer, heart); //Drop hearts
	...
	destroy_compiled_sprite(eggImg[0]);
	destroy_compiled_sprite(heartImg[0]);
	destroy_compiled_sprite(potionImg[0]);
	destroy_bitmap(buffer);
	destroy_bitmap(splatBuffer);
	destroy_bitmap(background);
	destroy_bitmap(hpHeart);
	destroy_bitmap(splat);
	destroy_bitmap(sgCollisionMask);
	destroy_sample(bg);
